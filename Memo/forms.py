...
from wtforms import StringField, TextAreaField, SubmitField, EmailField, PasswordField, HiddenField, FileField, BooleanField, RadioField
from flask_wtf.file import FileAllowed
from flask_login import current_user
from wtforms.validators import DataRequired, Length, ValidationError, EqualTo, Optional, Email
...
class LoginForm(FlaskForm):
    email = StringField('メールアドレス：', validators=[DataRequired('メールアドレスは必須入力です。')])
    password = PasswordField('パスワード：', validators=[DataRequired('パスワードは必須入力です。')])
    remember = BooleanField('ログイン状態を1週間保持し自動でマイページに移動する')
    submit = SubmitField('ログイン')

class SignUpForm(LoginForm): # ログイン処理と同じなため機能を継承
    username = StringField('ユーザー名', validators=[DataRequired(message='ユーザー名を入力してください'), Length(max=50)])
    email = EmailField('メールアドレス', validators=[DataRequired(message='メールアドレスを入力してください'), Email(message='メールアドレスの形式が正しくありません')])
...
class EditUserForm(FlaskForm):
    ...
    confirm_password = PasswordField('パスワード確認再入力', validators=[Optional(), EqualTo('password', message='パスワードが一致しません')])
    # サムネイル更新
    thumbnail = FileField("サムネイル画像", validators=[Optional(), FileAllowed(["jpg", "jpeg", "png"], "画像ファイルのみ") ])
    preset_thumbnail = RadioField('既存画像', choices=[(f'{i:02}.png', f'{i:02}.png') for i in range(1, 11)], validators=[Optional()])
