...
## Blueprint単位のエラーハンドリング

API全体で一貫したエラーレスポンス形式を返すため、Blueprint単位でカスタムエラーハンドラーを定義します。
...
def unprocessable(e):
    return jsonify({'error': 'Validation failed', 'details': str(e), 'code': 422}), 422
...
Blueprint単位でエラーハンドラーを設定することで、APIルートのみに影響を限定できます。500エラー時は必ず `db.session.rollback()` を呼んでDBの状態をリセットすることを忘れないようにしましょう。

## APIバージョニングの設計

APIを長期運用する場合は、バージョン管理の設計が重要です。Blueprintを活用したバージョニングが一般的です。
...
v1とv2で異なるシリアライズ形式やビジネスロジックを持つことができ、既存クライアントへの後方互換性を維持しながら新機能を追加できます。

## レスポンスのシリアライズ

Flask-Marshmallowを使うとモデルとJSONの変換を宣言的に定義できます。
