...
Jinja2には組み込みフィルター（`upper`, `truncate`, `join` など）の他に、独自フィルターを登録できます。

```python
# app.py または filters.py
...
テンプレートでは `{{ article.content | markdown | safe }}` のように使用できます。`safe` フィルターを忘れるとHTMLタグがエスケープされてしまうため注意が必要です。デコレータ形式でも登録できます。
...
すべてのテンプレートに共通の変数を渡したい場合は `context_processor` を使います。
...
def inject_globals():
    ...
    site_name = current_app.config.get('SITE_NAME', 'Tech Blog')
    return {'categories': categories, 'SITE_NAME': site_name}
```

これにより、すべてのテンプレートで `{{ SITE_NAME }}` や `{{ categories }}` が自動で利用可能になります。グローバルナビのカテゴリー一覧など、複数ページで共通して必要なデータに最適です。

## グローバル関数の登録

`context_processor` が変数を渡すのに対し、`jinja_env.globals` では関数をテンプレートに渡せます。
...
テンプレートでは `{% if is_active('public.index') %}active{% endif %}` のように使用でき、ナビゲーションのアクティブ状態管理などに便利です。
